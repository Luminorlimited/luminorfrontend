...
export default function Users() {
  ...
  const getStatusColor = (isActivated?: string) => {
    if (!isActivated) return "text-gray-700";

    const statusLower = isActivated.toLowerCase();
    switch (statusLower) {
      case "active":
  ...
  const { data: getProfessional, isLoading: professionalLoading } =
    useGetProfessionalQuery(undefined);
  ...
  const UserTable = ({
    ...
    users: any;
    userType: "client" | "retireProfessional";
    currentPage: number;
    isLoading: boolean;
  }) => (
    <Table>
            ...
            <TableRow key={index}>
              ...
              <TableCell>
                ...
                >
                  <span
                    className={
                      user?.[userType]?.name?.lastName ? "" : "font-bold"
                    }
                  >
                    {user?.[userType]?.name?.firstName}{" "}
                    {user?.[userType]?.name?.lastName || "Unknown User"}
                  </span>
                </Link>
              </TableCell>
              <TableCell>
                <Image
                  ...
                  height={50}
                  src={
                    user?.profileUrl === "null" || user?.profileUrl == null
                      ? logo
                      : user?.profileUrl
                  }
                  alt={"my-logo"}
              ...
              <TableCell className="text-right">
                <Button
                  ...
                  onClick={() =>
                    handleDelete(
                      user?.client?._id || user?.retireProfessional?._id
                    )
                  }
                >
        ...
        <div className="rounded-md border">
          ...
          <div className="flex justify-between mt-4">
            ...
            <Button
              ...
              onClick={() => setCurrentPageClients(currentPageClients + 1)}
              disabled={
                currentPageClients * itemsPerPage >= (clients?.length || 0)
              }
            >
              Next
            </Button>
        ...
        <div className="rounded-md border">
          ...
          <div className="flex justify-between mt-4">
            <Button
              className="bg-bg_primary rounded-[8px] hover:bg-[#21205fef]"
              onClick={() =>
                setCurrentPageProfessionals(currentPageProfessionals - 1)
              }
            ...
            <Button
              ...
              disabled={
                currentPageProfessionals * itemsPerPage >=
