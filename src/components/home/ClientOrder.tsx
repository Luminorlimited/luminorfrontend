  ...
  return (
    <div className="container mx-auto  py-8 px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {clientOrder?.data &&
          clientOrder?.data.map((order: any, index: any) => (
            <Card
              key={index}
              className="overflow-hidden rounded-[10px] border border-gray-200 hover:shadow-md transition-shadow flex flex-col justify-between"
            >
              <div
                className="h-2 w-full"
                style={{
                  backgroundColor:
                    getStatusColor(order?.transaction?.paymentStatus).replace(
                      "bg-",
                      ""
                    ) === "bg-primary"
                      ? "#5633D1"
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  >
                    {order?.transaction?.paymentStatus.charAt(0).toUpperCase() +
                      order?.transaction?.paymentStatus.slice(1)}
                  </Badge>
                </div>
                ...
                <div className="space-y-3 mb-4">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-gray-700">
                      {order?.orderReciver?.name?.firstName}{" "}
                      {order?.orderReciver?.name?.lastName}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-gray-700">
                      {" "}
                      <b> Created on </b> {DeliveryformatDate(order?.createdAt)}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-sm font-medium">
                    <span className="text-primary">
                      $
                      {Number.parseFloat(order?.transaction?.amount).toFixed(2)}
                    </span>
                  </div>
                </div>
              </CardContent>
              ...
              <CardFooter className="px-6 py-3">
                <Link href={`/clientOrder/${order._id}`} className="w-full">
                  <Button className="w-full bg-primary hover:bg-[#4628b0] rounded-[7px]">
                    View Details
                  </Button>
                </Link>
              </CardFooter>
