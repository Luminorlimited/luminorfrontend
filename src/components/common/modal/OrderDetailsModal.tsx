...
import offer from "@/assets/images/mainlogo.png";
...
const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({
  ...
  const deleteOffer = async () => {
    try {
      ...
      toast.success(
        "Offer cancellation has been sent to the Retired Professional."
      );
      onClose();
    } catch (error) {
      // console.log(error);
      if (error) toast.error("Failed to cancel offer");
    }
  };
  ...
  return (
    <div className="w-full  items-center justify-center z-50">
      <div className="">
        ...
        <div className=" bg-[#F8FAFB] mb-4 rounded-md ">
          <Image
            src={offer}
            alt="Offer Image"
            width={400}
        ...
        <div className="text-sm  bg-[#F8FAFB] p-2">
          <p className="mb-2 flex justify-between">
            ...
            <span className="text-black">
              {data?.pofessionalInfo?.name?.firstName}{" "}
              {data?.pofessionalInfo?.name?.lastName}{" "}
            </span>
          </p>
          <p className="mb-2 flex justify-between">
        ...
        <div className="flex justify-between mt-6 gap-2">
          <button
            onClick={deleteOffer}
            className="rounded-[12px]  w-full text-[16px]  font-medium text-black bg-slate-200 hover:bg-slate-300 hover:shadow-sm transition-colors duration-200"
          >
            Decline
          </button>
          <Button
