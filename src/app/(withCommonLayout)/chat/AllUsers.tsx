...
interface AllUsersProps {
  handleshowMessage: (email: string) => void;
...
export default function AllUsers({ handleshowMessage, getUser }: AllUsersProps) {
  ...
  console.log(`My all Conversation`, getConversation?.data?.[0].name);
    ...
    <div>
      <div className="w-full max-w-md mx-auto bg-white rounded-lg">
        ...
        <div>
          ...
          {getConversation?.data?.map((conversation:any, index:number) => (
            <li
            key={index}
            onClick={() =>handleshowMessage(conversation?.email)}
          ...
          >
            <div className="relative">
              <Image
                src={conversation.profileImg || demoimg}
                alt="Avatar"
                width={48}
                height={48}
