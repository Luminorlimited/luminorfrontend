...
const Page: React.FC = () => {
  ...
  const { data: orderList, isLoading: loading } =
    useTransactionListQuery(undefined);
  const { data: ProfessionalorderList, isLoading: loading2 } =
    useProfessionalOrderQuery(undefined);
  ...
  return (
    <div className="p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {ProfessionalorderList?.data ? (
          token?.role === "client" ? (
            ProfessionalorderList?.data?.map(
              (order: any) =>
                order.project ? ( // Check if order.project exists
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600">
                      Serial Number:{" "}
                      <strong>{order?.project?.totalPrice || ""}</strong>{" "}
                    </p>
                    <p className="text-sm text-gray-600">
                      Payment Status:{" "}
          ...
          <div className="h-80 col-span-3 flex w-full items-center justify-center">
            {loading || loading2 ? "Loading..." : "  No orders available."}
          </div>
        )}
      </div>
    </div>
